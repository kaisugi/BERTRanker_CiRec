...
class BiEncoderRanker(torch.nn.Module):
    ...
    def global_score_candidate(
        ...
        token_idx_glbl, segment_idx_glbl, mask_glbl = to_bert_input(
            global_info_vecs, self.NULL_IDX
        )
        # global info: right side
        _, embedding_glbl = self.model(
             None, None, None, token_idx_glbl, segment_idx_glbl, mask_glbl
        )
        
        embedding_redu = embedding_glbl + embedding_ctxt

        # Candidate encoding is given, do not need to re-compute
        # Directly return the score of context encoding and candidate encoding
        if cand_encs is not None:
            return embedding_redu.mm(cand_encs.t())
        ...
        return embedding_redu.mm(embedding_cands.t())


    def forward(self, global_info_input, context_input, cand_input):
        if global_info_input is None:
            ...
            scores = self.global_score_candidate(global_info_input, context_input, cand_input)
            bs = scores.size(0)
