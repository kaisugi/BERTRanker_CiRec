...
def evaluate(
    ...
    for step, batch in enumerate(iter_):
        ...
        if params["global_info"]:
            global_info_input, context_input, candidate_input, _ = batch
        else:
            context_input, candidate_input, _ = batch
        with torch.no_grad():
            if params["global_info"]:
                eval_loss, logits = reranker(global_info_input, context_input, candidate_input)
            else:
                eval_loss, logits = reranker(None, context_input, candidate_input)
...
def main(params):
    ...
    logger.info(train_samples[:1])
    logger.info("Read %d train samples." % len(train_samples))

    train_data, train_tensor_data = data.process_citation_data(
        train_samples,
        tokenizer,
    ...
    valid_data, valid_tensor_data = data.process_citation_data(
        valid_samples,
        tokenizer,
        params["max_context_length"],
        params["max_cand_length"],
    ...
    for epoch_idx in trange(int(num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(iter_):
            ...
            if params["global_info"]:
                global_info_input, context_input, candidate_input, _ = batch
                loss, _ = reranker(global_info_input, context_input, candidate_input)
            else:
                context_input, candidate_input, _ = batch
                loss, _ = reranker(None, context_input, candidate_input)
